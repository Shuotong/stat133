...
#m <- <your code here>
m <- matrix(rexp(100, rate = 3), nrow = 10, ncol = 10, byrow = F)
...
#t <- <your code here>
t1 <- infants[infants$parity == 1, ]
t2 <- t1[t1$marital == "Married", ]
t <- table(t2$ed)
...
first.cache = c()
for (i in 1:500){
  first.cache = c(first.cache, unlist(Cache500[i])[1])
}
