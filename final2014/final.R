...
# a random permutation of the even numbers from 2 to 222
  set.seed(31415)
v <- sample(c(1:111)*2, size = 111, replace = FALSE)
...
plot(iris$Petal.Width ~ iris$Petal.Length, xlab = "Petal Length", ylab = "Petal Width", col = iris$Species)
...
plot(infants$bwt ~ infants$gestation,xlab = "infant birthweight in ounces", ylab = "gestation time in days",col = 'red',pch = '*')
abline(v = 259)
# [6 pts]
...
# Add x-axis labels
hist(infants$age, freq = FALSE, xlab = "Mother's Age")
...
minchin.split <- as.vector(tolower(unlist(strsplit(minchin, " "))))
